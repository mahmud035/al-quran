...
Define full-text search over the user's active translation edition — query handling, debounce, result capping, deep-linking a result to its ayah, and the required loading, empty, and error states — alongside the retained surah-name matching in the same input.
